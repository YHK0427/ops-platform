...
const ROUND_TYPE_COLORS: Record<string, string> = {
    ...
    COMBINED: "bg-amber-500/15 text-amber-400 border-amber-500/30",
};
        ...
        >
            <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-semibold text-sm text-white truncate">{round.title}</h3>
                    <span className={cn(
                    ...
                    {visibleTabs.map((tab) => (
                        <button
                            ...
                            className={cn(
                                ...
                                activeTab === tab.key
                                    ? "text-white bg-[var(--color-accent)]/15 border border-[var(--color-accent)]/30"
                                    : "text-[var(--color-text-secondary)] bg-white/5 border border-white/10 hover:bg-white/10"
                            )}
                        >
                            {tab.label}
                        </button>
...
function RoundsTab({
    ...
    function getStatusColor(round: EvalRound) {
        if (round.results_open) return "bg-emerald-500/15 text-emerald-400 border-emerald-500/30";
        if (round.is_open) return "bg-blue-500/15 text-blue-400 border-blue-500/30";
        if (round.closed_at) return "bg-slate-500/15 text-slate-400 border-slate-500/30";
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="grid gap-3">
                    {rounds.map((round) => (
                        <motion.div
                            key={round.id}
                            layout
                            className="rounded-xl border border-[var(--color-border)] bg-zinc-900/50 backdrop-blur-md p-4"
                        >
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                    ...
                                    {isAdmin && (
                                        ...
                                        >
                                            {round.is_open ? (
                                                ...
                                                <LockOpen className="w-3.5 h-3.5 text-green-400" />
                                            )}
                                        </Button>
                                    )}
                                    ...
                                    {isAdmin && (
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            className="h-8 text-xs text-[var(--color-text-secondary)] hover:text-white"
                                    ...
                                    {isAdmin && (
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-8 w-8 text-rose-400/60 hover:text-rose-400"
            ...
            >
                <AlertDialogContent className="bg-zinc-900 border-[var(--color-border)]">
                    <AlertDialogHeader>
...
function CreateRoundDialog({
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); else onOpenChange(v); }}>
            <DialogContent
                className={cn(
                    "bg-zinc-900 border-[var(--color-border)]",
        ...
        <div className="space-y-4">
            ...
            {!selectedRoundId ? (
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    {/* Self eval (members) */}
                    <div className="rounded-xl border border-[var(--color-border)] bg-zinc-900/50 p-4">
                        <div className="flex items-center justify-between mb-4 pb-3 border-b border-[var(--color-border)]">
                            <h3 className="text-sm font-semibold text-white">자기 평가 (기수)</h3>
                            <span className={cn(
                    ...
                    <div className="rounded-xl border border-[var(--color-border)] bg-zinc-900/50 p-4">
                        <div className="flex items-center justify-between mb-4 pb-3 border-b border-[var(--color-border)]">
                            <h3 className="text-sm font-semibold text-white">청중 평가 (운영진)</h3>
                            <span className={cn(
                                "text-xs font-medium px-2 py-0.5 rounded-full",
                                        ...
                                        <div key={evaluatorName}>
                                            ...
                                            <div className="space-y-0.5 pl-1">
                                                {evalAssignments.map((a) => (
                                                    <div
                                                        key={a.id}
                                                        className="flex items-center gap-2 px-1.5 py-1 rounded hover:bg-white/5"
                                                    >
                                                        {a.submitted_at ? (
