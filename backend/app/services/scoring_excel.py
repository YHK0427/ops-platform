...
from app.models import (
    ScoringComment, ScoringParticipant, ScoringRank, ScoringRound, ScoringScore,
)
...
ROLE_KR = {"JUDGE": "심사위원", "OBSERVER": "참관위원"}
...
async def generate_scoring_excel(db: AsyncSession, rnd: ScoringRound) -> io.BytesIO:
    ...
    head = ["순위", "팀", "심사위원", "참관위원", "총점"] + [c.label for c in criteria]
    ws.append([])
    ws.append(head)
    hrow = ws.max_row
    ...
    for r in computed:
        row = [
            r.rank, tname.get(r.target_id, "?"),
            r.judge_points, r.observer_points, r.total,
        ] + [r.criterion_avg.get(c.id, 0) for c in criteria]
        ws.append(row)
    ...
    _autosize(ws, [6, 22, 11, 11, 9] + [12] * len(criteria))
